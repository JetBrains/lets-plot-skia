...
object AutoMpg {
    private fun s(s: String) = s
    private fun i(s: String) = s.toInt()
    private fun d(s: String) = s.toDouble()

    private val columns = linkedMapOf<String, (String) -> Any>(
        "miles per gallon" to ::d,
        "number of cylinders" to ::i,
        "engine displacement (cu. inches)" to ::d,
        "engine horsepower" to ::i,
        "vehicle weight (lbs.)" to ::d,
        "time to accelerate (sec.)" to ::d,
        "model year" to ::i,
        "origin of car" to ::s,
        "vehicle name" to ::s,
    )
...
31.0,4,119.0,82,2720,19.4,82,US,chevy s-10"""
    ...
    fun map(): Map<String, List<Any>> {
        ...
        for (colName in columns.keys) {
            result[colName] = ArrayList()
        }

        for (row in data) {
            val values = row.split(",")
            for ((i, col) in columns.toList().withIndex()) {
                val v = col.second(values[i])
                (result[col.first] as ArrayList).add(v)
            }
        }
